<?php
	...
	{
		$mysqli = new mysqli('127.0.0.1', 'areanet_admin', 'erSS1979_', 'areanet_caribbeancup');
	...
	function group_detail($year, $group)
	{
		...
		for ($num_fila = 0; $num_fila <= $resultado->num_rows - 1; $num_fila++) 
		{
			$resultado->data_seek($num_fila);
			$fila = $resultado->fetch_assoc();
			$script = $script . '	<li><p>' . $fila['name'] . '</p><p><a href="http://www.area1650.net/caribbeancup/country_stats.html?country=' . $fila['code'] . '">stats</a></p></li>';
		}
		$script = $script . '</ul>';
		$mysqli->close();
		return $script;
	...
	function group_matches($year, $group)
	{
		...
		for ($num_fila = 0; $num_fila <= $resultado->num_rows - 1; $num_fila++) 
		{
			...
			$script = $script . '<td><a href="http://www.area1650.net/caribbeancup/match_stats.html?squada=' . $fila['squada'] . '&squadb=' . $fila['squadb'] . '">stats</a></td>';
			$script = $script . '</tr>';
		}
		$script = $script . '</table>';
		$mysqli->close();
	...
	function country_stats($country)
	{
		try
		{
			$conn = new PDO('mysql:host=127.0.0.1;dbname=areanet_caribbeancup', 'areanet_admin', 'erSS1979_');
		...
		$query = $query . '     on a.newsquad = :country and a.oldsquad = b.squad and b.matchid = h.matchid and b.time_type in (2,3,4,6) and h.game_type between 1 and 29 and h.matchdate < now()';
		$query = $query . ' left join game_score c';
		...
		$script = '<a href="http://www.area1650.net/caribbeancup/page.php">Caribbean Cup 2017</a>';
		$script = $script .'<table>';
		$record = 0;
	...
	function match_stats($squada, $squadb)
	{
		...
		$query = $query . ' 	 and b.time_type in (2,3,4,6) and h.game_type between 1 and 29 and h.matchdate < now()';
		$query = $query . ' left join game_score c';
