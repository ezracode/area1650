<?php
	...
	{
		$mysqli = new mysqli('127.0.0.1', 'areanet_admin', 'erSS1979_', 'areanet_ofcnationscup');
	...
	function group_detail($year, $group)
	{
		...
		for ($num_fila = 0; $num_fila <= $resultado->num_rows - 1; $num_fila++) 
		{
			$resultado->data_seek($num_fila);
			$fila = $resultado->fetch_assoc();
			$script = $script . '	<li><p>' . $fila['name'] . '</p><p><a href="http://www.area1650.net/ofcnationscup/country_stats.html?country=' . $fila['code'] . '">stats</a></p></li>';
		}
		$script = $script . '</ul>';
		$mysqli->close();
		return $script;
	...
	function group_matches($year, $group)
	{
		...
		for ($num_fila = 0; $num_fila <= $resultado->num_rows - 1; $num_fila++) 
		{
			...
			$script = $script . '<td><a href="http://www.area1650.net/ofcnationscup/match_stats.html?squada=' . $fila['squada'] . '&squadb=' . $fila['squadb'] . '">stats</a></td>';
			$script = $script . '</tr>';
		}
		$script = $script . '</table>';
		$mysqli->close();
	...
	function country_stats($country)
	{
		try
		{
			$conn = new PDO('mysql:host=127.0.0.1;dbname=areanet_ofcnationscup', 'areanet_admin', 'erSS1979_');
		...
		$script = '<a href="http://www.area1650.net/ofcnationscup/page.php">2016 OFC Nations Cup</a>';
		$script = $script .'<table>';
		$record = 0;
